...
public class Use_of_p_java_continue {
	public static void main(String[] args) {
		...
		for (int j = 1; j <= 8; j++) {
			if (j > 3 && j % 2 == 1) {
				continue; // 5 and 7 is skipped because of continue feature
				// if you use continue, this condition will only be skipped
				// continue used inside condition, it shows error if you use outside condition
				// continue;
				// you cannot use 2 continue or 2 break inside one condition, see line 17, 32
			}
			System.out.println(j); // 1, 2, 3, 4, 6, 8
		}
		...
		for (int j = 1; j <= 8; j++) {
			...
			System.out.println("Even Number: " + j); // 2, 4, 6, 8
		}
		...
		while (j <= 9) {
			if (j == 5) {
				j += 2; // important info: this is an exception, because the outcome 5, doesn't go to
						// print and increment in next line,
				// we use increment in line 64, so, the value of J become 7 from 5
				// important: if you comment out line 62: the code don't proceed further because
		...
		do {
			if (l == 5) {
				l = l + 2; // this is an exception, because the outcome doesn't go to print and increment
